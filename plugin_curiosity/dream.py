...
DREAM_TARGET = (
    ...
    "1. mission_get for your current mission; wiki_toc and look at each "
    "page's age_days (server-computed days since last edit). THE GATE: if "
    "every page has age_days >= 1, today was a quiet day — reply 'quiet "
    "night — nothing to consolidate' and stop (no wiki writes, no "
    "share_thought). Trust age_days over how the content looks: pages with "
    "age_days >= 1 were already consolidated by previous dreams, even if "
    "you can't see those dreams from here.\n"
